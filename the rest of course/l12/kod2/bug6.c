...
/* Perform my section of the dot product */
   ...
   // Sekcja krytyczna - czas na mutex
   pthread_mutex_lock(&MutexSumy);
   sum += SumaTemp;
   printf("thread: %ld done. Global sum now is=%li\n",tid,sum);
   pthread_mutex_unlock(&MutexSumy);
 ...
 */
 pthread_mutex_init(&MutexSumy, NULL); 
...
pthread_mutex_destroy(&MutexSumy);
free (a);
free (b);
pthread_exit(NULL);
}   
